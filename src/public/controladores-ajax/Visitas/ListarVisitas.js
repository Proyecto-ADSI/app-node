...
$(function () {
    ...
    columns: [
      ...
      },
      {
        ...
        render: function (data) {
          if (data === "1") {
            return `   
           <button id="BtnDetallesVI" class="btn btn-sm btn-primary" data-toggle="modal"  data-target=".ModalDetallesVisitas" title="Detalles">
           <i class="fa  fa-eye"></i></button>

            <button id="BtnCancelar" class="btn btn-sm btn-danger"  title="Cancelar">
  ...
  $("#DataTableVisitas tbody").on("click", "#BtnDetallesVI", function () {
   let DetalleVisitas = DataTableVisitas.row($(this).parents("tr")).data();

    DetallesVisitas(DetalleVisitas);
  });

  $("#DataTableVisitas tbody").on("click", "#BtnCancelar", function () {
    let DataVisita = DataTableVisitas.row($(this).parents("tr")).data();

    CancelarVisita(DataVisita);
  });
});

const RecargarDataTableVi = () =>{

  DataTableVisitas.ajax.reload();

}

const CancelarVisita = DataVisita =>{

  const Id_Visita = parseInt(DataVisita.Id_Visita);
  ...
  $.ajax({
    url: `${URL}/Visitas/CambioEstado` ,
    type: "patch",
    dataType: 'json',
    contentType: "application/json",
    data: JSON.stringify(CancelarVisita),
    processData:false,
    beforeSend: function(){
      MostrarLoaderPuntos();
    },
    success: function(Response){
      ...
      if(Response.data.Ok){
        ...
        swal("Perfecto","La visita ha sido cancelada", "success")
